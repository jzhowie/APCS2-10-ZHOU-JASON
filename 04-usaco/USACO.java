...
public static int bronze(String filename) throws FileNotFoundException {
	...
	for (int[] row : field) {
		for (int col : row) {
			if (elevation - col > 0) totalDepth = totalDepth + (elevation - col);
		}
	}
...
private static int[][] bronzeHelper(int row, int col, int depth, int[][] board) {
	...
	for (int i = row - 1; i < row + 2; i++) {
		for (int j = col - 1; j < col + 2; j++) {
			if (board[i][j] > board[coords[0]][coords[1]]) coords = new int[] {i, j};
		}
	}
	...
	for (int i = row - 1; i < row + 2; i++) {
		for (int j = col - 1; j < col + 2; j++) {
			if (board[i][j] > board[coords[0]][coords[1]]) board[i][j] = board[coords[0]][coords[1]];
		}
	}
	return board;
}
	...
	for (int sec = 0; sec < time; sec++) {
		for (int row_t = 0; row_t < rows; row_t++) {
			for (int col_t = 0; col_t < cols; col_t++) {
				...
				if (temp > 0) field[sec+1][row_t][col_t] = temp;
			}
		}
	}
...
private static long silverHelper(int row, int col, long[][] board) {
	long counter = 0;
	if (board[row][col] == -1) return -1;
	if (board[row][col] != 0) return 0;

	if (col+1 < board[0].length && board[row][col+1] != -1) counter += board[row][col+1];
	if (col-1 >= 0 && board[row][col-1] != -1) counter += board[row][col-1];
	if (row-1 >= 0 && board[row-1][col] != -1) counter += board[row-1][col];
	if (row+1 < board.length && board[row+1][col] != -1) counter += board[row+1][col];
